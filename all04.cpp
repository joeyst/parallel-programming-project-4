...
#define NUM_ELEMENTS_PER_CORE (ARRAYSIZE / NUMTHREADS)
...
main( int argc, char *argv[ ] )
{
        ...
        fprintf( stderr, "%4d\t, %12d\t", NUMTHREADS, ARRAYSIZE );
        ...
        for( int t = 0; t < NUMTRIES; t++ )
        {
                ...
                #pragma omp parallel
                {
                        int thisThread = omp_get_thread_num();
                        int first = thisThread * NUM_ELEMENTS_PER_CORE;
                        SimdMul( A, B, C, NUM_ELEMENTS_PER_CORE );
                }
                double time1 = omp_get_wtime( );
                double perf = (double)ARRAYSIZE / (time1 - time0);
                if( perf > maxPerformance )
        ...
        for( int t = 0; t < NUMTRIES; t++ )
        {
                ...
                #pragma omp parallel reduction(+:sumn)
                {
                        int thisThread = omp_get_thread_num();
                        int first = thisThread * NUM_ELEMENTS_PER_CORE;
                        sumn = NonSimdMulSum( A, B, NUM_ELEMENTS_PER_CORE );
                }
                double time1 = omp_get_wtime( );
                double perf = (double)ARRAYSIZE / (time1 - time0);
                if( perf > maxPerformance )
        ...
        for( int t = 0; t < NUMTRIES; t++ )
        {
                ...
                #pragma omp parallel reduction(+:sums)
                {
                        int thisThread = omp_get_thread_num();
                        int first = thisThread * NUM_ELEMENTS_PER_CORE;
                        sums = SimdMulSum( A, B, NUM_ELEMENTS_PER_CORE );
                }
                double time1 = omp_get_wtime( );
                double perf = (double)ARRAYSIZE / (time1 - time0);
                if( perf > maxPerformance )
